function AlbumCard({ data }) {
  ...
  return (
    ...
    <div className="font-inter border border-white/30 max-w-4xl mx-auto bg-black/80 text-white/80 rounded-lg shadow-lg p-6 mt-6 transition-all duration-300 hover:shadow-xl hover:scale-[1.02] m-4">
      <div className="flex flex-col md:flex-row items-start gap-6">
        <img
          src={data.cover_xl}
          alt={`${data.title} cover art`}
          className="w-64 h-64 object-cover rounded-lg border-4 border-white/30"
        />
        <div className="flex-1">
          ...
          <p className="text-white/80 mb-1">{data.nb_tracks} tracks</p>
          ...
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="inline-block bg-white/20 hover:bg-white/30 text-white/80 px-4 py-2 rounded-lg transition"
          >
            Listen on Deezer
          </a>
        </div>
        ...
        {contributors.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {contributors.map((contributor) => (
              <div
                key={contributor.id}
                className="bg-white/10 p-4 rounded-lg flex items-center gap-4 shadow"
              >
                <img
                ...
                />
                <div>
                  <p className="font-medium text-white/80">{contributor.name}</p>
                  <a
